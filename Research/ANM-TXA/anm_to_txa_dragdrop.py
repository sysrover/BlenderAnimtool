#!/usr/bin/env python3
"""
ANM to TXA Converter - Drag and Drop Version
Converts DayZ binary .anm animation files to text .txa format.
Simply drag ANM files onto this executable to convert them.
"""
...
# Usage: python anm_to_txa_dragdrop.py input.anm [output.txa]
# Or drag input.anm onto the executable
...
def _write_txa(anm, anim_name: str, out_path: pathlib.Path):
    ...
    with StringIO() as fs:
        ...
        for b in anm.bones:
            ...
            # Detect animated channels (have any intermediate keys beyond boundaries)
            def _is_dynamic(frames_list: List[int]) -> bool:
                if not frames_list:
                    return False
                # if any key not at boundary frames, consider dynamic
                for f in frames_list:
                    if f not in (0, max_frame):
                        return True
                return False
            ...
            pos_dynamic = _is_dynamic(pos_frames_sorted)
            rot_dynamic = _is_dynamic(rot_frames_sorted)
            scale_dynamic = _is_dynamic(scale_frames_sorted)

            # Build helper function to interpolate between keyframes
            def _interpolate_value(frame_num: int, key_map: Dict, is_quat: bool = False):
                """Interpolate or return a keyframe value. Returns None if no data is available."""
                if frame_num in key_map:
                    return key_map[frame_num]
                
                # Find surrounding keyframes
                sorted_frames = sorted(key_map.keys())
                
                # Before first keyframe or no keyframes
                if not sorted_frames or frame_num < sorted_frames[0]:
                    return None
                
                # After last keyframe
                if frame_num > sorted_frames[-1]:
                    return key_map[sorted_frames[-1]]
                ...
                if prev_frame is None or next_frame is None:
                    # Should not happen given the above checks
                    return None
                
                # Interpolate between prev_frame and next_frame
                prev_val = key_map[prev_frame]
                next_val = key_map[next_frame]
                
                frame_span = next_frame - prev_frame
                t = (frame_num - prev_frame) / frame_span
                
                if is_quat:
                    return prev_val.slerp(next_val, t)
                else:
                    return prev_val.lerp(next_val, t)

            # Build frame data for all frames first to detect consecutive identical frames
            frame_data = []  # List of (pos, rot, scale) tuples for each frame
            for f in range(0, max_frame + 1):
                # Only interpolate dynamic channels; for static channels, only use keyframe values
                if pos_dynamic:
                    frame_pos = _interpolate_value(f, pos_map, is_quat=False)
                else:
                    frame_pos = pos_map.get(f)
                
                if rot_dynamic:
                    frame_rot = _interpolate_value(f, rot_map, is_quat=True)
                else:
                    frame_rot = rot_map.get(f)
                
                if scale_dynamic:
                    frame_scale = _interpolate_value(f, scale_map, is_quat=False)
                else:
                    frame_scale = scale_map.get(f)

                frame_data.append((frame_pos, frame_rot, frame_scale))

            # Emit frames, collapsing consecutive identical frames into ranges
            f = 0
            while f <= max_frame:
                # Find the end of the range of identical frames
                range_end = f
                while range_end < max_frame and frame_data[range_end + 1] == frame_data[f]:
                    range_end += 1

                # Write frame header (with range if applicable)
                if range_end > f:
                    fs.write(f"   $frame {f} {range_end} {{\n")
                else:
                    fs.write(f"   $frame {f} {{\n")
...
def convert_anm_to_txa(input_path: str) -> bool:
    """Convert ANM file to TXA. Returns True on success."""
    try:
        inp = pathlib.Path(input_path).resolve()
        
        if not inp.exists():
            print(f"Error: File not found: {inp}")
            return False
        
        if inp.suffix.lower() != ".anm":
            print(f"Error: Please provide an ANM file. Received: {inp.name}")
            return False
        ...
        print(f"✓ Converted: {inp.name} → {outp.name}")
        return True
...
def main():
    # Convert all files passed as arguments (from drag-drop)
    if len(sys.argv) > 1:
        for file_path in sys.argv[1:]:
            convert_anm_to_txa(file_path)
        sys.exit(0)
    else:
        print("ANM to TXA Converter")
        print("Usage: Drag ANM files onto this executable to convert them")
        sys.exit(0)
