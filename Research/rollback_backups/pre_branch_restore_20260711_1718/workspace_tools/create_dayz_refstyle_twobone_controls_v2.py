...
OUT_BLEND = r"P:\Animation_Weapon\Weapon_template_aks74u_refstyle_twobone_controls_v2.blend"
REPORT = os.path.join(ROOT, "anm", "dayz-refstyle-twobone-controls-v2-report.json")

EXPORT_ARMATURE = "_DayZ_Character"
RIG = "DAT_DayZ_RefStyleTwoBoneControls"
PREFIX = "DAT_REF2B_"
...
def create_control_shapes():
    ...
    elbow = make_wire_shape(
        "WGT_DAT_Elbow_Diamond",
        [(0, -0.06, 0), (0.06, 0, 0), (0, 0.06, 0), (-0.06, 0, 0)],
        [(0, 1), (1, 2), (2, 3), (3, 0)],
    )
    return hand, elbow
...
def create_rig(export):
    ...
    with bpy.context.temp_override(object=rig, active_object=rig, selected_objects=[rig]):
        bpy.ops.object.mode_set(mode="OBJECT")
    hand_shape, elbow_shape = create_control_shapes()
    bpy.context.view_layer.objects.active = rig
    rig.select_set(True)
    with bpy.context.temp_override(object=rig, active_object=rig, selected_objects=[rig]):
    ...
    for pb in rig.pose.bones:
        if pb.name.startswith("CTRL_Hand."):
            ...
            pb.custom_shape_scale_xyz = (0.7, 0.7, 0.7)
...
def setup_constraints(export, rig, pole_angles):
    for side, spec in SIDES.items():
        ...
        add_copy(export.pose.bones[spec["arm"]], rig, f"OUT_Arm.{side}", "COPY_" + spec["arm"])
        add_copy(export.pose.bones[spec["arm_roll"]], rig, f"OUT_ArmRoll.{side}", "COPY_" + spec["arm_roll"])
        add_copy(export.pose.bones[spec["fore"]], rig, f"OUT_ForeArm.{side}", "COPY_" + spec["fore"])
        add_copy(export.pose.bones[spec["fore_roll"]], rig, f"OUT_ForeArmRoll.{side}", "COPY_" + spec["fore_roll"])
        add_copy(export.pose.bones[spec["hand"]], rig, f"OUT_Hand.{side}", "COPY_" + spec["hand"])
...
def add_bake_text():
    old = bpy.data.texts.get("DAT_REF2B_BAKE")
    if old:
        bpy.data.texts.remove(old)
    text = bpy.data.texts.new("DAT_REF2B_BAKE")
    text.write(
        "# DAT_REF2B_BAKE\n"
        "import bpy\n"
        f"BONES = {TRACKED!r}\n"
...
def show_scene(export, rig):
    ...
    for b in rig.data.bones:
        b.hide = b.name.startswith("DRV_") or b.name.startswith("OUT_")
    for obj in bpy.context.scene.objects:
        obj.select_set(False)
    rig.select_set(True)
    bpy.context.view_layer.objects.active = rig
