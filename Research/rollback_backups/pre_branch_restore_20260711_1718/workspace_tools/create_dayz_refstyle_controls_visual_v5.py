...
BASE_BLEND = r"P:\Animation_Weapon\Weapon_template_aks74u_refstyle_rotation_tweak_v4.blend"
OUT_BLEND = r"P:\Animation_Weapon\Weapon_template_aks74u_refstyle_controls_visual_v5.blend"
REPORT = os.path.join(ROOT, "anm", "dayz-refstyle-controls-visual-v5-report.json")

EXPORT_ARMATURE = "_DayZ_Character"
RIG = "DAT_DayZ_RefStyleControlsVisual"
PREFIX = "DAT_REFVIS_"
...
def create_control_shapes():
    ...
    elbow = make_wire_shape(
        "WGT_DAT_Elbow_Circle",
        circle_points(0.075, 32),
        loop_edges(32),
    )
    joint = make_wire_shape(
        "WGT_DAT_Joint_Flat_Circle",
        circle_points(0.055, 32),
        loop_edges(32),
    )
    roll = make_wire_shape(
        "WGT_DAT_Roll_Flat_Rect",
        [(-0.085, -0.018, 0), (0.085, -0.018, 0), (0.085, 0.018, 0), (-0.085, 0.018, 0)],
        loop_edges(4),
    )
...
def create_rig(export):
    ...
    for pb in rig.pose.bones:
        if pb.name.startswith("CTRL_Hand."):
            ...
            pb.custom_shape_scale_xyz = (0.75, 0.75, 0.75)
