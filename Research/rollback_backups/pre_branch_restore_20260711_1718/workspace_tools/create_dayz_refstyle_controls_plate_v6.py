...
BASE_BLEND = r"P:\Animation_Weapon\Weapon_template_aks74u_refstyle_controls_visual_v5.blend"
OUT_BLEND = r"P:\Animation_Weapon\Weapon_template_aks74u_refstyle_controls_plate_v6.blend"
REPORT = os.path.join(ROOT, "anm", "dayz-refstyle-controls-plate-v6-report.json")

EXPORT_ARMATURE = "_DayZ_Character"
RIG = "DAT_DayZ_RefStyleControlsPlate"
PREFIX = "DAT_REFPLATE_"
...
def make_plate_shape(name, verts, edges, faces):
    ...
    mat.diffuse_color = (0.15, 0.95, 1.0, 0.18)
...
def create_control_shapes():
    ...
    hand = make_plate_shape(
        "WGT_DAT_Hand_Plate_Clickable",
        hand_verts,
        loop_edges(8),
        [tuple(range(8))],
    )
    elbow_verts = circle_points(0.105, 40)
    elbow = make_plate_shape(
        "WGT_DAT_Elbow_Disc_Clickable",
        elbow_verts,
        loop_edges(40),
        [tuple(range(40))],
    )
    joint_verts = circle_points(0.085, 40)
    joint = make_plate_shape(
        "WGT_DAT_Joint_Disc_Clickable",
        joint_verts,
        loop_edges(40),
        [tuple(range(40))],
    )
    roll = make_plate_shape(
        "WGT_DAT_Roll_Rect_Clickable",
        [
            (-0.13, -0.035, 0.0),
            (0.13, -0.035, 0.0),
            (0.13, 0.035, 0.0),
