...
OUT_BLEND = r"P:\Animation_Weapon\Weapon_template_aks74u_refbake_previewrig_v4_native.blend"
REPORT = os.path.join(ROOT, "anm", "dayz-refbake-previewrig-v4-native-report.json")
...
def create_preview_mesh(export_obj, preview_obj):
...
def add_native_preview_constraints(preview, rig):
    added = []
...
def add_bake_text():
    text = bpy.data.texts.get("DAT_NATIVE_PREVIEW_BAKE") or bpy.data.texts.new("DAT_NATIVE_PREVIEW_BAKE")
    text.clear()
    text.write(
        """# DAT_NATIVE_PREVIEW_BAKE
# Preview is live through native Child Of constraints on DAT_PreviewSkeleton.
# _DayZ_Character stays clean until you call apply_export_once/bake_export_*.
import bpy
...
def measure_neutral_delta(export_obj, preview):
    import math
...
def main():
    ...
    if not export_obj or not rig:
        raise RuntimeError("Missing export armature/control rig")

    removed_handlers = remove_bad_handlers()
    removed_preview = remove_preview_objects()
    clean_old_texts()
    reset_controls(rig)
    bpy.context.view_layer.update()
    ...
    constraints = add_native_preview_constraints(preview, rig)
    ...
    result = {
        ...
        "neutral_delta": measure_neutral_delta(export_obj, preview),
        ...
        "helper_text": "DAT_NATIVE_PREVIEW_BAKE",
    }
